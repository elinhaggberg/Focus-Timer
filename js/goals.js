import { getLogEntries } from "./storage.js";
...
// Only completed focus sessions count toward a Focus goal — finished to-do
// lists are tracked separately in the Log but don't move this needle.
function focusDaySet(entries) {
  const set = new Set();
  for (const e of entries) {
    if (e.kind === "focus") set.add(startOfDay(e.completedAt));
  }
  return set;
}

function countWeek(weekStart, today, focusDays) {
  let count = 0;
  for (let i = 0; i < 7; i++) {
    const day = weekStart + i * DAY_MS;
    if (day > today) break;
    if (focusDays.has(day)) count++;
  }
  return count;
}
...
// already hit) where enough distinct focus days were logged.
// Daily goals: streak counts consecutive scheduled weekdays (going backward
// from today) that had a logged session, skipping over an unfinished today.
export function computeGoalStatus(goal, logEntries = getLogEntries()) {
  const now = Date.now();
  const today = startOfDay(now);
  const focusDays = focusDaySet(logEntries);
  const goalStart = goal.createdAt ? startOfDay(goal.createdAt) : today;

  if (goal.type === "weekly") {
    const target = goal.timesPerWeek || 1;
    const thisWeekStart = startOfWeek(now);
    const count = countWeek(thisWeekStart, today, focusDays);
    ...
    while (cursor >= earliestWeek) {
      const c = countWeek(cursor, today, focusDays);
      if (isCurrent) {
        isCurrent = false;
        if (c >= target) streak++;
      } else if (c >= target) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    occurred++;
    if (focusDays.has(day)) satisfied++;
  }
  ...
  while (cursor >= goalStart) {
    ...
    if (activeDays[idx]) {
      const hit = focusDays.has(cursor);
      if (isToday && !hit) {
